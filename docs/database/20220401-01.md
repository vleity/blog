...
```shell
sqlldr 用户/密码@ip:port/sid control=sqlldr.ctl log=sqlldr.log rows=1000
```
...
SELECT S.SID,
		 ...
		 S.INST_ID,
	     S.*
    FROM GV$LOCKED_OBJECT LO, GV$SESSION S, DBA_OBJECTS OBJ, GV$PROCESS P
...
| 序号 | 表名            | 描述                 |
| :--: | --------------- | -------------------- |
|  1   | v$session       | 记录会话及锁的信息   |
|  2   | v$session_wait  | 记录会话的等待信息   |
|  3   | v$lock          | 锁信息               |
|  4   | v$locked_object | 当前被锁对象的信息   |
|  5   | dba_locks       | 对V$lock的格式化视图 |
|  6   | dba_blockers    | 正在阻塞资源的会话   |
|  7   | dba_waiters     | 正在等待锁资源的会话 |
|  8   | v$latch         | 锁存器信息           |
|  9   | v$latch_misses  | latch争用的丢失统计  |
...
--表结构差异，字段名及顺序
DECLARE
  v_tab1 VARCHAR2(100) := 't1';
  v_tab2 VARCHAR2(100) := 't2';
BEGIN
...
```plsql
SELECT A.ID,A.C,A.CD,A.UD,A.ST,A.START_DATE,
       CASE WHEN B.ID IS NOT NULL AND A.END_DATE = '9999-12-31' THEN TO_CHAR(SYSDATE-1,'YYYY-MM-DD')
				 ELSE A.END_DATE END END_DATE
FROM LEITY_TEST_ZP A  --拉链表
     LEFT JOIN LEITY_TEST_2 B ON A.ID = B.ID  --增量表
UNION ALL
SELECT A.*,TO_CHAR(SYSDATE,'YYYY-MM-DD') START_DATE,'9999-12-31' END_DATE
FROM LEITY_TEST_2 A;  --增量表


SELECT A.ID,A.C,A.CD,A.UD,A.ST,A.START_DATE,
       CASE WHEN B.ID IS NULL OR A.END_DATE < '9999-12-31' THEN A.END_DATE
				 ELSE TO_CHAR(SYSDATE-1,'YYYY-MM-DD') END END_DATE
FROM LEITY_TEST_ZP A  --拉链表
     LEFT JOIN LEITY_TEST_2 B ON A.ID = B.ID  --增量表
UNION ALL
SELECT A.*,TO_CHAR(SYSDATE,'YYYY-MM-DD') START_DATE,'9999-12-31' END_DATE
...
```sql
SELECT 姓名,SUM(CASE WHEN ID = '语文' THEN 分数 ELSE 0 END) 语文,
       SUM(CASE WHEN ID = '数学' THEN 分数 ELSE 0 END) 数学,
       SUM(CASE WHEN ID = '英语' THEN 分数 ELSE 0 END) 英语
FROM (
    select id,'A' 姓名,A 分数
    from test
    union all
